...
public interface RoleApi {
  ...
  /**
   * POST /api/v1/account/{account-id}/role Assign or revoke role
   *
   ...
   * @return OK (status code 200) or Created (status code 201) or Bad request. Data is invalid
   *     (status code 400) or Account doesn&#39;t exist (status code 404) or unexpected error
   *     (status code 200)
   */
  @RequestMapping(
  ...
  /**
   * GET /api/v1/account/{account-id}/role/{role} Returns account role id for given account id and
   * role
   *
   * @param accountId Id of account to get account role id (required)
   * @param role Role of account to get account role id (required)
   * @return OK (status code 200) or Account doesn&#39;t exist (status code 404)
   */
  @RequestMapping(
  ...
  ResponseEntity<AccountRoleIDReturnModel> apiV1AccountAccountIdRoleRoleGet(
      @PathVariable("account-id") UUID accountId, @PathVariable("role") RoleEnum role)
      throws Exception;
}
