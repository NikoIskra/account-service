...
public class AccountRoleValidatorTest {
  ...
  @InjectMocks AccountRoleValidator accountRoleValidator;
  ...
  void testValidateAccountRoleRequestData_roleNotFound() {
    when(accountRepository.existsById(uuid)).thenReturn(true);
    when(accountRoleRepository.existsByAccountIDAndRole(uuid, RoleEnum.CLIENT.getValue()))
        .thenReturn(false);
    AccountRoleRequestModel accountRoleRequestModel = createAccountRoleRequestModelStatusRevoked();
    assertThrows(
        NotFoundException.class,
  ...
  void testValidateAccountRoleGetRequest() {
    ...
    when(accountRoleRepository.findByAccountIDAndRole(uuid, RoleEnum.CLIENT.getValue()))
        .thenReturn(Optional.of(createAccountRoleActive()));
    assertDoesNotThrow(
  ...
  void testValidateAccountRoleGetRequest_accWithRoleRevoked() {
    when(accountRepository.existsById(uuid)).thenReturn(true);
    when(accountRoleRepository.findByAccountIDAndRole(uuid, RoleEnum.CLIENT.getValue()))
        .thenReturn(Optional.of(createAccountRole()));
